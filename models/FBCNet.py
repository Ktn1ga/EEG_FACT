...
-----------------------------------------------------
Model Name:         FBCNet
Repository URL:     https://github.com/ravikiran-mane/FBCNet
Original Paper:     Ravikiran Mane, Effie Chew, Karen Chua, Kai Keng Ang, Neethu Robinson, A.P. Vinod, Seong-Whan Lee, and Cuntai Guan,
...
class FBCNet(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for i in range(x.shape[0]):
            ...
            x_all.append(torch.tensor(xi))
        x = torch.stack(x_all,dim=0)
        x = x.permute(0,3,1,2).to(device).type(torch.float32)
...
def main():
    ...
    model = FBCNet(num_electrodes=channels,chunk_size=samples,num_classes=4)
    out = model(input)
    print('===============================================================')
    print('out', out.shape)
    print('model', model)
